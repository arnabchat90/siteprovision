...
export default class SiteProvisioningTool extends React.Component<
  ISiteProvisioningToolProps,
  ISiteProvisioningToolState
> {
  constructor(
    props: ISiteProvisioningToolProps,
    state: ISiteProvisioningToolState
  ) {
    super(props);
    this.state = {
      loadForm: false,
    ...
    this.createDocumentLibrariesJSOM = this.createDocumentLibrariesJSOM.bind(
      this
    );
    this.readLibraryConfigurationList = this.readLibraryConfigurationList.bind(
      this
    );
    this.createFolderStructure = this.createFolderStructure.bind(this);
    this.createCustomPermissionLevels = this.createCustomPermissionLevels.bind(
      this
    );
  ...
  private getListsTitles(siteUrl): void {
    ...
    context.executeQueryAsync(
      (sender: any, args: SP.ClientRequestSucceededEventArgs): void => {
        ...
        this.setState(
          (
            prevState: ISiteProvisioningToolState,
            props: ISiteProvisioningToolProps
          ): ISiteProvisioningToolState => {
  ...
  private async addItemToProjectsList(
    ...
    var pmUser =
      self.state.formData.pm[0] == null ||
      self.state.formData.pm[0] == undefined
        ? null
        : website.ensureUser(self.state.formData.pm[0].id);
    var execUser =
      self.state.formData.executive[0] == null ||
      self.state.formData.executive[0] == undefined
        ? null
        : website.ensureUser(self.state.formData.executive[0].id);
    var spUser =
      self.state.formData.sp[0] == null ||
      self.state.formData.sp[0] == undefined
        ? null
        : website.ensureUser(self.state.formData.sp[0].id);
    context.load(website);
    if (pmUser !== null) context.load(pmUser);
    if (execUser !== null) context.load(execUser);
    if (spUser !== null) context.load(spUser);
  ...
  private readLibraryConfigurationList(
        ...
        clientContext.executeQueryAsync(
          (sender: any, args: SP.ClientRequestSucceededEventArgs): void => {
            ...
            while (listEnumerator.moveNext()) {
              ...
              for (var i = 1; i <= 10; i++) {
                if (
                  listItem.get_item("Level" + i) !== "" &&
                  listItem.get_item("Level" + i) !== null &&
                  listItem.get_item("Level" + i) !== undefined
                ) {
        ...
        clientContext.executeQueryAsync(
          ...
          (sender: any, args: SP.ClientRequestFailedEventArgs): void => {
            self.setState({
              ...
              currentStatus:
                "There is some error while provisioning your site" +
                args.get_message(),
  ...
  private createDocumentLibrariesJSOM(siteUrl, newSiteColURL): Promise<any> {
    return new Promise<any>(
      (
        ...
        const newContext: SP.ClientContext = new SP.ClientContext(
          newSiteColURL
        );
        const newlists: SP.ListCollection = newContext.get_web().get_lists();

        //name of doc library hard coded.
        this.readLibraryConfigurationList(
          context,
          "DocumentLibraryStructure"
        ).then((items: any) => {
          console.log(items);
  ...
  private createFolderStructure(
    element: any,
    newlists: SP.ListCollection,
    newContext: SP.ClientContext,
    resolve: (itemObjects: any) => void,
    self: this,
    index,
    ...
    newContext.executeQueryAsync(
      ...
      (sender: any, args: SP.ClientRequestFailedEventArgs): void => {
        ...
        const list: SP.List = newContext
          .get_web()
          .get_lists()
          .getByTitle(element.LibraryName);
  ...
  createSharePointGroups(
    newSiteUrl: string,
    spGroupPermissionMapping: any[]
  ): Promise<any> {
    var self = this;
    return new Promise<any>(
      (
        ...
        var loop = function(i) {
          self.createSharePointGroupinSP(
            newContext,
            spGroupPermissionMapping[i].SPGroupName,
            spGroupPermissionMapping[i].SPGroupName,
            spGroupPermissionMapping[i].CustomPermissionName,
            function() {
              if (++i < spGroupPermissionMapping.length) {
                self.setState({
                  currentStatus:
                    "Adding Domain Groups Permissions - " +
                    spGroupPermissionMapping[i].SPGroupName,
                  error: null,
                  messageBarType: MessageBarType.info
                });
                ...
                //act.SharePoint.SharePointAppProgress.completed(true, "Completed");
                self.setState({
                  currentStatus: "All Domain Group Added",
                  error: null,
                  messageBarType: MessageBarType.info
                });
                resolve(true);
            ...
            function(error) {
              ...
              self.setState({
                showCurrentStatus: true,
                currentStatus: "There is some error - " + error.message,
                messageBarType: MessageBarType.error
              });
            }
          );
  ...
  createCustomPermissionLevels(newSiteUrl: string): Promise<any> {
    ...
    return new Promise<any>(
      (
        ...
        const newContext: SP.ClientContext = new SP.ClientContext(newSiteUrl);
        var customPermissionLevelNames = [
          "FULL ACCESS",
          "Edit With Delete",
          "Edit With No Delete",
          "Read Only"
        ...
        customPermissionLevelNames.forEach(item => {
          dsReadPermissions.push({
            levelName: item,
            permissions: this.createPermissionSet(item)
          });
        });
        self.setState({
        ...
        var loop = function(i) {
          self.createCustomPermission(
            newContext,
            dsReadPermissions[i].levelName,
            dsReadPermissions[i].levelName,
            dsReadPermissions[i].permissions,
            function() {
  ...
  createCustomPermission(context, name, desc, permissions, success, fail) {
    ...
    var roleDefinition = context
      .get_site()
      .get_rootWeb()
      .get_roleDefinitions()
      .add(roleDefinitionCreationInfo);
    context.executeQueryAsync(success, fail);
  }

  createSharePointGroupinSP(context, userName, desc, roleName, success, fail) {
    var web: SP.Web = context.get_web();
    ...
    // assign the user to the new RoleDefinitionBindingCollection.
    roleAssignments.add(user, rolDefColl);
  ...
  createSiteCollection(formData: IFormData, finishCallback: Function) {
    var self = this;
    self.setState({ formData: formData });
    if (
      formData.siteName == null ||
      formData.siteName == undefined ||
      formData.siteName == ""
    ) {
      self.setState({
        showCurrentStatus: true,
        currentStatus: "Site Name can not be empty",
        messageBarType: MessageBarType.error
      });
      return;
    }
    ...
    if (
      formData.siteOwners == null ||
      formData.siteOwners == undefined ||
      formData.siteOwners.length == 0
    ) {
      self.setState({
        showCurrentStatus: true,
        currentStatus:
          "Please provide a Site Collection Administrator to provision the site",
        messageBarType: MessageBarType.error
      });
      return;
    }
    ...
    if (
      formData.siteDescription == null ||
      formData.siteDescription == undefined ||
      formData.siteDescription == ""
    ) {
      self.setState({
        showCurrentStatus: true,
        currentStatus: "Site Description can not be empty",
        messageBarType: MessageBarType.error
      });
      return;
    }
    ...
    self.setState({
      showCurrentStatus: true,
      currentStatus: "Provisioning your site",
      messageBarType: MessageBarType.info,
      finishDisabled: true
    });
    ...
    self
      .createNewSiteCollectionUsingGraph(self, siteCreationBody)
      .then(response => {
      ...
      .then(
        data => {
          ...
          self.setState({
            showCurrentStatus: true,
            currentStatus:
              "Created Site Collection, looking for the site id...",
            messageBarType: MessageBarType.info
          });
          var groupId = data.id;
          setTimeout(function() {
            self
              .getSiteCollectionIdFromGroupId(self, groupId)
              .then(response => {
                return response.json();
              })
              .then(data => {
                self.setState({
                  showCurrentStatus: true,
                  currentStatus: "Got the Site Collection ID",
                  messageBarType: MessageBarType.info
                });
                console.log(data);
                if (data.error !== null && data.error !== undefined) {
                  self.setState({
                    showCurrentStatus: true,
                    currentStatus:
                      "There is some error - " + data.error.message,
                    messageBarType: MessageBarType.error
                  });
                  return;
                }
                ...
                self
                  .createDocumentLibrariesJSOM(
                    self.props.context.pageContext.web.absoluteUrl,
                    data.webUrl
                  )
                  .then(data => {
                    //self.getListsTitles(self.state.currentCreatedSiteUrl);
                    self
                      .createCustomPermissionLevels(
                        self.state.currentCreatedSiteUrl
                      )
                      .then(() => {
                        ...
                        self
                          .readPermissionList(context, "PermissionAssignment")
                          .then(groupPermissionMappings => {
                            ...
                            self
                              ...
                              .then(() => {
                                ...
                                self.setState({
                                  showCurrentStatus: true,
                                  currentStatus:
                                    "Your brand new team site has been created",
                                  messageBarType: MessageBarType.success,
                                  finishDisabled: false,
                                  clearForm: true
                                });
                                ...
                                Object.assign(document.createElement("a"), {
                                  target: "_blank",
                                  href: self.state.currentCreatedSiteUrl
                                }).click();
                              });
                          });
                      });
  ...
  private createDocumentLibraries(
    self: this,
    siteCollectionId: any,
    listCreationBody: string
  ) {
    return self.props.context.aadHttpClientFactory
      ...
      .then((client: AadHttpClient) => {
        ...
        return client.post(
          `https://graph.microsoft.com/v1.0/sites/${siteCollectionId}/lists`,
  ...
  private createNewSiteCollectionUsingGraph(
    self: this,
    siteCreationBody: string
  ) {
    return self.props.context.aadHttpClientFactory
      .getClient("https://graph.microsoft.com")
      .then((client: AadHttpClient) => {
  ...
  public render(): React.ReactElement<ISiteProvisioningToolProps> {
    ...
    return (
      <div className={styles.siteProvisioningTool}>
        {this.state.loadForm == false ? (
          <div className={styles.container}>
            <div className={styles.row}>
              <div className={styles.column}>
                <span className={styles.title}>
                  Create a site : {this.props.context.pageContext.web.title}
                </span>
              </div>
            </div>
